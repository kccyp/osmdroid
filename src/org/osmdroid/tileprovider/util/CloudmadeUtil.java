			...
			synchronized (mToken) {
				...
				if (mToken.length() == 0) {
					...
					try {
						...
						if (DEBUGMODE) {
							Log.d("daryu-osmdroid","Response from Cloudmade auth: " + response.getStatusLine());
						}
						if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
							...
							if (DEBUGMODE) {
								Log.d("daryu-osmdroid","First line from Cloudmade auth: " + line);
							}
							mToken = line.trim();
							if (mToken.length() > 0) {
								...
								Log.e("daryu-osmdroid","No authorization token received from Cloudmade");
							}
						}
					} catch (final IOException e) {
						Log.e("daryu-osmdroid","No authorization token received from Cloudmade: " + e);
					}
				}
			}
		}
